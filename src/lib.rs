    ...
    fn sim_microphone(p: &Position, a: &Angle, t: &Microphone) -> f64 {
        match t {
            ...
            _ => {
                ...
                let gain = (PI / 2.0 - a.theta).sin() * (vartheta).sin() * (a.phi - varphi).cos() + (PI / 2.0 - a.theta).cos() * (vartheta).cos();
                rho + (1.0 - rho) * gain
            }
        }
    }
                        ...
                        for q in 0..=1 {
                            for j in 0..=1 {
                                for k in 0..=1 {
                                    ...
                                    if (2*mx-q).abs()+(2*my-j).abs()+(2*mz-k).abs() <= n_order || n_order == -1 {
...
mod tests {
    ...
    fn test_example() {
        let _ = compute_rir(
            340.0,
            16000.0,
            &vec![Position{ x: 2.0, y: 1.5, z: 2.0 }],
            4096,
            &Position{ x: 2.0, y: 3.5, z: 2.0 },
            &Length{ x: 5.0, y: 4.0, z: 6.0 },
            &Betas{ x: [0.4, 0.4], y: [0.4, 0.4], z: [0.4, 0.4], },
            &Microphone::Omnidirectional,
            -1,
            &Angle{ phi: 0.0, theta: 0.0 },
            true,
        );
    }
}
